...
ADDED: Gemini AI fallback, context tracking, status tracking integration
"""
import logging
import re
...
from app.config import GEMINI_API_KEY, RATES, MINIMUM_CHARGE, SERVICE_META
...
AREA_PROMPT = """\
📐 *Enter your field area in hectares:*
...
Minimum charge: ₹1,500 per booking.
...
CROP_MAP = {
    "1": "rice",   "2": "wheat", "3": "cotton", "4": "sugarcane",
    "5": "vegetables", "6": "fruits", "7": "pulses", "8": "other",
}
SERVICE_MAP = {
    "1": "pesticide_spray",  "2": "crop_monitoring",
    "3": "field_mapping",    "4": "soil_analysis",   "5": "fertilizer_spray",
}
SERVICE_RATES = {
    "pesticide_spray": 1200, "crop_monitoring": 800, "field_mapping": 1500,
    "soil_analysis": 2000,   "fertilizer_spray": 1000,
}
...
_AI_TRIGGERS = re.compile(
    ...
    r"pesticide|crop|harvest|soil|yield|price|cost|acre|hectare|problem|issue)",
    re.IGNORECASE
)
...
def _create_service_request(phone: str, data: dict, db: Session) -> ServiceRequest:
    ...
    if not cust.district or cust.district == "Unknown":
        cust.district = data.get("location", "Unknown")
    ...
    req = ServiceRequest(
        ...
        field_location=data.get("location", "Tamil Nadu"),
...
def _order_summary(data: dict) -> str:
    ...
    return (f"📋 *BOOKING SUMMARY — Please confirm:*\n\n"
            ...
            f"📐 Area:           *{data.get('area_hectares')} ha*\n"
            f"📍 Location:       *{data.get('location')}*\n"
            f"🌾 Crop:           *{data.get('crop_type','').replace('_',' ').title()}*\n"
...
def _process(phone: str, text: str, db: Session) -> str:
    ...
    if state == "enter_area":
        ...
        try:
            area = float(text)
            if area < 0.5: return "⚠️ Minimum area is 0.5 hectares.\n\n" + AREA_PROMPT
            if area > 500: return "⚠️ Maximum area is 500 ha. Contact us.\n\n" + AREA_PROMPT
            data["area_hectares"]  = area
            cost = max(round(area * data["rate"] / 50) * 50, MINIMUM_CHARGE)
            data["estimated_cost"] = cost
            session["state"]       = "select_district"
            reply = f"✅ Area: *{area} ha* · Cost: *₹{cost:,}*\n\n" + DISTRICT_MENU
        except ValueError:
    ...
    if state == "select_district":
        if text in DISTRICT_MAP:
            data["location"] = DISTRICT_MAP[text]; session["state"] = "select_crop"
            reply = f"✅ Location: *{data['location']}*\n\n" + CROP_MENU
        elif text == "9":
            session["state"] = "enter_custom_district"; reply = "Please type your district name:"
        elif text == "0":
    ...
    if state == "enter_custom_district":
        if len(text) < 2: return "Please enter a valid district name:"
        data["location"] = text_raw.title(); session["state"] = "select_crop"
        reply = f"✅ Location: *{data['location']}*\n\n" + CROP_MENU
        _log_chat(phone, "bot", reply, cust.id, db); db.commit()
        return reply
    ...
    if state == "select_crop":
        if text in CROP_MAP:
            data["crop_type"] = CROP_MAP[text]; session["state"] = "select_date"
            reply = _date_menu()
        elif text == "0":
